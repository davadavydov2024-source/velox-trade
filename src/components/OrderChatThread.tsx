...
import { Send, Star, XCircle, ChevronRight } from "lucide-react";
...
export function OrderChatThread({ orderId, counterpartName }: { orderId: string; counterpartName: string }) {
  ...
  useEffect(() => {
    ...
    // (см. OrderDealTimeline/DealSheet), и не давать подтвердить получение до этого шага.
  ...
  // бот-посредник, и когда продавец передаёт предмет напрямую (см. фикс в
  // api/deliveries/submit-nickname и OrderDealTimeline). Пока delivery ещё грузится
