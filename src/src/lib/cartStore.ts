...
// так что localStorage здесь работает нормально и переживает обновление страницы). Для залогиненных
// пользователей она ЕЩЁ и дублируется в Firestore (см. CartSync) — нужно только для того, чтобы
// сервер мог раз в день проверить брошенные корзины и напомнить о них, самой покупке это не мешает.
