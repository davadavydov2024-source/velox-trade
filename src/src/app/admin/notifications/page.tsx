  ...
  async function handlePushBroadcast(e: React.FormEvent) {
    ...
    try {
      ...
      if (data.total === 0) {
        toast(
          "warning",
          "Подписчиков с активным доступом нет. Если недавно меняли VAPID-ключи — старые подписки могли " +
            "автоматически удалиться как несовместимые (см. предыдущее сообщение об ошибке, если оно было). " +
            "Попроси пользователей заново включить push в «Профиль → Безопасность»."
        );
