...
export default function StaffSupportPage() {
  ...
  const hasAccess = !!user && (isAdminUid(user.uid) || profile?.staffRole === "manager");
  ...
  async function refresh() {
    ...
    try {
      ...
      if (err?.code === "permission-denied") {
        toast("error", "Нет прав на чтение обращений — обратись к администратору за ролью «Менеджер».");
      }
    } finally {
      setLoading(false);
    }
  ...
  if (!hasAccess) {
    return <p className="text-center text-white/40 py-16">Эта страница доступна только сотрудникам с ролью «Менеджер».</p>;
  }
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-white/40">Ответы отправляются от имени администрации — как обычно видит пользователь.</p>
      </div>
              ...
              <div className="space-y-3 mb-4 max-h-96 overflow-y-auto">
                {active.messages.map((m, i) => (
                  <div key={i} className={`flex ${m.from === "admin" ? "justify-end" : "justify-start"}`}>
                    ...
                    >
                      <p className="text-[10px] text-white/30 mb-0.5">{m.from === "admin" ? "Ты" : active.userName}</p>
                      {m.text}
                    </div>
                  </div>
                ))}
