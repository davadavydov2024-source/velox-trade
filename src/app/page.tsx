...
import { getGames, getFeaturedProducts } from "@/lib/products";
...
export default function HomePage() {
  ...
  return (
    <div>
      ...
      <section className="relative overflow-hidden border-b border-border">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-20 md:py-28 grid md:grid-cols-2 gap-12 items-center relative">
          <div>
            ...
            <p className="text-white/50 mb-8 max-w-md text-lg">{t("home_subtitle")}</p>
            <div className="flex flex-wrap gap-3 mb-10">
              <Link href="/catalog" className="btn-primary px-6 py-3.5 flex items-center gap-2 shadow-glow">
                {t("home_cta_shop")} <ArrowRight size={18} />
              </Link>
            ...
            <div className="flex gap-8">
              {[
                { label: t("home_stat_safe"), value: "100%" },
                { label: t("home_stat_support"), value: "24/7" },
                { label: t("home_stat_delivery"), value: t("home_stat_delivery_value") },
              ].map((stat) => (
                <div key={stat.label}>
      ...
      {/* Featured (paid boost tier 2) */}
      {featured.length > 0 && (
        <section className="max-w-7xl mx-auto px-4 sm:px-6 pt-16">
          <h2 className="text-2xl font-bold mb-6">{t("home_featured_title")}</h2>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 py-16">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-2xl font-bold">{t("home_popular_games")}</h2>
