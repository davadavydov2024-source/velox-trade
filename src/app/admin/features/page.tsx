...
export default function AdminFeaturesPage() {
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      ...
      <div className="card p-5">
        <p className="font-medium text-sm mb-1">Минимальная цена в заявке «Продать предметы»</p>
        <p className="text-xs text-white/40 mb-3">
          Пользователь не сможет отправить заявку на продажу дешевле этой суммы.
        </p>
        <div className="relative w-40">
          <input
            type="number"
            min={0}
            value={flags.minSellPrice}
            onChange={(e) => setFlags((f) => ({ ...f, minSellPrice: Math.max(0, Number(e.target.value)) }))}
            className="input-field py-2.5 pr-8"
          />
          <span className="absolute right-3 top-1/2 -translate-y-1/2 text-white/40 text-sm">₽</span>
        </div>
      ...
      <div className="card p-5">
        <p className="font-medium text-sm mb-1">Продвижение товаров</p>
        <p className="text-xs text-white/40 mb-4">
          Платное продвижение, которое продавец может купить со своего баланса на странице «Мои товары» для
          собственного товара. Тир 1 закрепляет товар в топе каталога его игры. Тир 2 делает то же самое и вдобавок
          показывает товар в блоке «Рекомендуемые» на главной странице.
        </p>
        <div className="grid sm:grid-cols-2 gap-4">
          <div>
            <p className="text-xs text-white/50 mb-2">Тир 1 — закреп в игре</p>
            <div className="flex gap-2">
              ...
              <div className="relative w-24">
                ...
                />
                <span className="absolute right-3 top-1/2 -translate-y-1/2 text-white/40 text-sm">дн.</span>
              </div>
            </div>
          </div>
          <div>
            <p className="text-xs text-white/50 mb-2">Тир 2 — закреп + главная</p>
            <div className="flex gap-2">
              <div className="relative flex-1">
                <input
                  type="number"
