...
function ChatsInner() {
  ...
  const setThreadOpen = useChatUi((s) => s.setThreadOpen);

  // Прячем нижнюю мобильную панель навигации, пока открыт конкретный чат (иначе на iPhone она
  // перекрывает поле ввода сообщения) — и обязательно возвращаем её обратно при уходе со страницы.
  useEffect(() => {
    setThreadOpen(view !== null);
